...
void UGameManager::Init()
{
	...
	if (UEngine* Engine = GetEngine())
	{
		...
		Engine->OnNetworkFailure().AddUObject(this, &UGameManager::HandleNetworkFailure);

		// 서버 주소가 잘못됬다, 서버 인원이 꽉 찼다, 맵 파일이 없다.
		Engine->OnTravelFailure().AddUObject(this, &UGameManager::HandleTravelFailure);
	}
}
...
void UGameManager::JoinServer(FString IPAddress)
{
...
void UGameManager::DisconnectServer()
{
	FString MapName;
...
void UGameManager::HandleNetworkFailure(
...
void UGameManager::HandleTravelFailure(
